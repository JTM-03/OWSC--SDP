...
import { ArrowLeft, Clock, CheckCircle, Package, Filter, Loader2, XCircle, CreditCard, ExternalLink } from "lucide-react";
...
export function OrderManagement({ onBack }: OrderManagementProps) {
  ...
  const changePaymentStatus = async (orderId: number, paymentStatus: 'Paid' | 'Unpaid') => {
    try {
      await orderAPI.updatePaymentStatus(orderId, paymentStatus);
      toast.success(`Order #${orderId} marked as ${paymentStatus}`);
      fetchOrders();
      // Update the selected order dialog if it's open
      if (selectedOrder && selectedOrder.id === orderId) {
  ...
  /**
   * Derive a simple Paid / Unpaid label from the payments array.
   * A receipt upload sets paymentStatus to 'Pending' (awaiting verification).
   * Admin marking it Paid sets it to 'Completed'.
   */
  const getPaymentLabel = (order: Order): 'Paid' | 'Receipt Uploaded' | 'Unpaid' => {
    const p = order.payments?.[0];
    if (!p) return 'Unpaid';
    if (p.paymentStatus === 'Completed') return 'Paid';
    if (p.receiptUrl) return 'Receipt Uploaded';
    return 'Unpaid';
  };
  ...
  const getPaymentColor = (label: string) => {
    switch (label) {
      ...
      case 'Receipt Uploaded': return 'bg-amber-100 text-amber-800 border-amber-200';
                  ...
                  {filteredOrders.length === 0 ? (
                    ...
                    filteredOrders.map(order => (
                      <TableRow key={order.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setSelectedOrder(order)}>
                        ...
                        {/* Payment status badge — green=Paid, amber=Receipt Uploaded, red=Unpaid */}
                        <TableCell>
                          <Badge variant="outline" className={getPaymentColor(getPaymentLabel(order))}>
          ...
          {selectedOrder && (
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center py-2 border-t">
                ...
                <span className="font-semibold">Rs. {Number(selectedOrder.subtotalAmount ?? selectedOrder.totalAmount).toLocaleString()}</span>
              </div>

              {/* Service fee only applies to Dine-in orders */}
              {selectedOrder.orderType === 'Dine-in' && (
                <div className="flex justify-between items-center py-2">
                  <span className="font-semibold text-foreground">Service Fee (10%)</span>
                  <span className="font-semibold text-orange-600">Rs. {Number(selectedOrder.serviceFee ?? 0).toLocaleString()}</span>
                </div>
              )}
              ...
              <div className="border rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className={getPaymentColor(getPaymentLabel(selectedOrder))}>
                    {getPaymentLabel(selectedOrder)}
                  </Badge>
                </div>

                {/* Show receipt link if member uploaded one */}
                {selectedOrder.payments?.[0]?.receiptUrl && (
                  <a
                    href={selectedOrder.payments[0].receiptUrl}
                    target="_blank"
                ...
                {/* Admin/staff can toggle between Paid and Unpaid */}
                <div className="flex gap-2 pt-1">
                  <Button
                    size="sm"
                    variant={getPaymentLabel(selectedOrder) === 'Paid' ? 'default' : 'outline'}
                    className={getPaymentLabel(selectedOrder) === 'Paid' ? 'bg-green-600 hover:bg-green-700 text-white flex-1' : 'flex-1'}
                    onClick={() => changePaymentStatus(selectedOrder.id, 'Paid')}
                    disabled={getPaymentLabel(selectedOrder) === 'Paid'}
                  >
                    Mark as Paid
                  </Button>
                  <Button
                    size="sm"
                    variant={getPaymentLabel(selectedOrder) === 'Unpaid' ? 'default' : 'outline'}
                    className={getPaymentLabel(selectedOrder) === 'Unpaid' ? 'bg-red-600 hover:bg-red-700 text-white flex-1' : 'flex-1'}
                    onClick={() => changePaymentStatus(selectedOrder.id, 'Unpaid')}
                    disabled={getPaymentLabel(selectedOrder) === 'Unpaid'}
                  >
                    Mark as Unpaid
                  </Button>
                </div>
