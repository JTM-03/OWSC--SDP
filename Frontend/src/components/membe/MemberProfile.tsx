...
interface MemberData {
  ...
  address: string;
  city: string;
  membershipType: "full" | "associate" | "sport" | "social" | "lifetime";
...
export function MemberProfile({ onBack }: MemberProfileProps) {
    ...
    const fetchMemberProfile = async () => {
      try {
        ...
        const mappedData: MemberData = {
          ...
          city: "",
          membershipType: (membership?.type as any) || "full",
  ...
  const membershipTypes = {
    full: {
      ...
      benefits: ["Access to all facilities", "Voting rights", "Event bookings", "Guest privileges", "Priority support"]
    },
    associate: {
      ...
      benefits: ["Sports facilities", "Dining access", "Event discounts", "Limited guests"]
    },
    sport: {
      label: "Sport Member",
      ...
      icon: User,
      price: "Rs. 5,000/year",
      benefits: ["All sports facilities", "Coaching programs", "Tournament entry", "Basic dining"]
    },
    social: {
      label: "Social Member",
      ...
      icon: User,
      price: "Rs. 10,000/year",
      benefits: ["Restaurant & bar", "Social events", "Lounge access", "Special offers"]
    },
    lifetime: {
      ...
      benefits: ["All privileges forever", "Priority bookings", "VIP events", "Unlimited guests", "Transferable"]
    }
  };
  ...
  const activeMembershipConfig = membershipTypes[memberData.membershipType] || membershipTypes.full;
  ...
  const handleSaveProfile = async () => {
    ...
    if (editForm.phone && !phoneRegex.test(editForm.phone)) {
      toast.error("Phone number must be 10 digits starting with 07 or 011 (e.g. 07XXXXXXXX or 0112XXXXXX)");
      return;
    }
            ...
            <Card className="border-2 border-secondary/20">
              ...
              <CardContent className="pt-6">
                <div className="space-y-4">
                  ...
                  {upgradeStatus !== 'none' && upgradeStatus !== 'completed' && memberData.membershipType !== "lifetime" && (
                    <Button
                  ...
                  {upgradeStatus === 'none' && memberData.membershipType !== "lifetime" && (
                    <Button
                      className="w-full bg-secondary text-primary hover:bg-secondary/90"
                      onClick={() => setUpgradeDialogOpen(true)}
                    >
